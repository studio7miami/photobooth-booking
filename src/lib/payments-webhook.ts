...
export async function handlePaymentsWebhook(req: Request, env: StripeEnv) {
...
export async function paymentsWebhookResponse(request: Request): Promise<Response> {
  const rawEnv = new URL(request.url).searchParams.get("env");
  if (rawEnv !== "sandbox" && rawEnv !== "live") {
    return Response.json({ received: true, ignored: "invalid env" });
  }
  try {
    await handlePaymentsWebhook(request, rawEnv);
